...
const BarPulse = ({ color }: { color: string }) => (
  <div className="flex gap-1 h-14 items-end opacity-60">
...
export default function NeuroAI_Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#C1D5C0] text-[#1E293B] font-serif" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
      ...
      {/* 🚀 MAIN CONTENT GRID (Pale Jade Context) */}
      <main className="max-w-[1400px] mx-auto p-4 md:p-8">
        ...
        {/* 🛡 FOOTER STATUS */}
        <footer className="mt-8 bg-[#1E293B] p-5 rounded-xl flex items-center justify-between text-white shadow-2xl border border-white/10">
          <div className="flex items-center gap-4">
            <ShieldCheck size={28} className="text-emerald-400"/>
            <div>
              <p className="text-xs font-bold uppercase tracking-widest">Neuro Core Linked</p>
              <p className="text-[11px] opacity-40 mt-1 font-serif">Encrypted Stream Protocol</p>
            </div>
          </div>
          <button className="bg-[#E5E5D8] text-[#1E293B] px-12 py-3 rounded-lg font-bold uppercase text-xs tracking-widest hover:bg-white transition-all active:scale-95 shadow-lg">
            Record Metrics
          </button>
        </footer>
      </main>
